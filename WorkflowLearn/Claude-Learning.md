...
- **Agent 流转时必须读取对应 Agent.md，不得跳过或手写替代**
...
1. 列出所有项目目录（`项目/*/`）
2. **排除归档项目**（见下方列表）
3. 读取每个项目的 `项目信息.md`（NOT 掌握卡片）
4. 从「已掌握节点」表格提取 `next_review` 字段
5. 筛选 `next_review <= 今天` 的节点
6. 按项目分组统计
7. 输出提醒，让用户选择

**优势**：
- ✅ 只读项目信息文件（2-3个），不读所有掌握卡片（10-20个）
- ✅ Token 节省 ~80%
- ✅ 复习信息已在 `项目信息.md` 维护，无需重复扫描
