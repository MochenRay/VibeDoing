...
## Lab 8：Wizard of Oz Research Plan

- 目标：为一个 AI feature 设计从零验证方案。
- 交付物：`woz-research-plan.md`
- 必须包含：目标用户、任务脚本、人工模拟边界、观察指标、信任问题、失败接管、是否值得继续自动化
- 通过条件：能在模型未稳定前验证产品假设，而不是先做完整技术实现。

## Lab 9：Streaming / Confidence UX Brief

- 目标：设计一个需要等待、部分结果或风险提示的 AI 输出界面。
- 交付物：`streaming-confidence-ux.md`
- 必须包含：流式节奏、部分结果处理、可取消、置信表达、引用/证据、失败状态
- 通过条件：用户能理解系统进度和不确定性，不把 UI 动效误读为结果可靠。
