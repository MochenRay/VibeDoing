...
stateDiagram-v2
    ...
    实操Agent --> 导师Agent: 连续错误>2次
    实操Agent --> 考官Agent: Practice Passed（自动）
    
    考官Agent --> 导师Agent: 验证未通过>2次
    考官Agent --> 课程Agent: 节点掌握，解锁下一个（自动）
...
1. **扫描所有掌握卡片**：
   - 遍历 `项目/*/掌握卡片/*.md`
   - 读取 frontmatter 中的 `next_review` 字段

2. **筛选到期节点**：`next_review <= 今天日期`
...
3. **若有到期节点**：
   ...
   以下节点今天需要复习：
   - [[掌握卡片/03. CAP定理]]（距上次复习 3 天）
   - [[掌握卡片/05. Raft算法]]（距上次复习 7 天）
   
   复习方式：快速费曼验证（60秒用自己的话讲清楚）
...
| `/实操` 或 `练一下` | 做微任务验证 | 加载实操Agent |
| `/验证` 或 `考我` | 费曼技巧测试 | 加载考官Agent |
...
**触发**：导师Agent自动流转 / 用户主动说"练习" / 指令 `/实操`

**执行**：
1. 读取 `_系统/实操Agent.md`
2. 执行：生成微任务 → 强制论述 → 变式题重考 → 通过则解锁
...
**触发**：实操Agent自动流转 / 用户说"考考我" / 指令 `/验证`

**前置条件**：必须通过实操Agent

**执行**：
1. 读取 `_系统/考官Agent.md`
2. 执行：费曼挑战 → 变式重考 → 确认掌握 → 更新复习计划
...
- 必须通过 **实操Agent** + **考官Agent** 双重验证，才能解锁下一节点
- 错误时给变式题，连续错误时回退重学
- 拒绝"似懂非懂"的浏览式学习
