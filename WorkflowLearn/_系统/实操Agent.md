...
- 用户在场景题中已经展示了"教别人"的能力

> [!NOTE]
> 费曼验证的核心目的是确认用户"真懂"而非"碰巧答对"。
> 如果掌握验证场景题已经充分验证了理解深度，费曼阶段可以跳过，避免冗余。
...
3. **验证与记录**：
   - **说得好**（通俗+准确）：
     - 肯定用户
     - **强制追加到 `掌握卡片`** → 创建/更新 `## 💡 验证高光` 章节
     - **物理证据**：必须调用 `replace_file_content` 或 `write_to_file`
   - **说不清/有漏洞**：
     - 追问引导
     - 若连续说不清 → 标记「原理模糊」，直接回退导师Agent

> [!CAUTION]
> **Physical Evidence Requirement (物理证据要求)**：
> - 费曼验证通过后，**必须**立即更新掌握卡片文件。
> - **严禁**仅在内存中记录或口头肯定。
> - **检查方式**：卡片文件的 `last_modified` 时间戳必须在验证后更新。
...
- [ ] **掌握卡片已更新**：包含 `## 💡 验证高光` 章节
- [ ] **项目信息已更新**：`已掌握节点` 列表新增当前节点
- [ ] **知识白板已更新**：Canvas 新增当前节点
- [ ] **文件时间戳验证**：卡片文件的 `last_modified` 在验证后

### 下一步流转
- [ ] **通过**：流转到课程Agent进行路径更新
- [ ] **回退**：流转到导师Agent，并附带归因记录
- [ ] **禁止跳过**：严禁直接进入下一个节点学习
